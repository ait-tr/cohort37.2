...
const playersWithGoals = players.map(element => {
    const copyElement = {...element}
    copyElement['goals'] = Math.ceil(Math.random()*20);
    return copyElement;
})
console.log(playersWithGoals);

const filtredPlayers = copyPlayers.filter((element) =>{
    return element.height > 180 && element.weight < 80;
   
})
console.log(filtredPlayers);

const filtredGoalsPlayers = playersWithGoals.sort((a,b) => a.goals - b.goals)
console.log(filtredGoalsPlayers);
